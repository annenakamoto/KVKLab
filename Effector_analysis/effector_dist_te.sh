...
while read GENOME; do
    ...
    bedtools closest -D a -N -id -t first -a SORTED/E_SCO_OG_${GENOME}.bed -b SORTED/E_SCO_OG_${GENOME}.bed > genes_genes.U.${GENOME}.bed
    bedtools closest -D a -N -iu -t first -a SORTED/E_SCO_OG_${GENOME}.bed -b SORTED/E_SCO_OG_${GENOME}.bed > genes_genes.D.${GENOME}.bed
    ### parse the data
    ...
    cat genes_genes.U.${GENOME}.bed | awk -v OFS='\t' '{ print $1, $2, $3, $4 }' | while read LINE; do
        ...
        us=$(grep "^${LINE}" genes_genes.U.${GENOME}.bed | awk '$9 == "-1" { print "none" } $9 != "-1" { print -$9 }')
        ds=$(grep "^${LINE}" genes_genes.D.${GENOME}.bed | awk '$9 == "-1" { print "none" } $9 != "-1" { print $9 }')
    ...
    cat genes_eff.U.${GENOME}.bed | awk -v OFS='\t' '{ print $1, $2, $3, $4 }' | while read LINE; do
        gene=$(echo ${LINE} | awk '{ print $4 }')
        us=$(grep "^${LINE}" genes_eff.U.${GENOME}.bed | awk '$9 == "-1" { print "none" } $9 != "-1" { print -$9 }')
        ds=$(grep "^${LINE}" genes_eff.D.${GENOME}.bed | awk '$9 == "-1" { print "none" } $9 != "-1" { print $9 }')
