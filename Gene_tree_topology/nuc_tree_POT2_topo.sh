...
GENE=$1 # a gene with a POT2 topo tree

# get the orthogroup the gene is in, filter the genes in it to only contain the representative genomes + outgroup
OG=$(grep ${GENE} GENOME_TREE/OrthoFinder_out/Results_Nov22/Orthogroups/Orthogroups.txt | awk '{ print substr($1, 1, length($1)-1) }')
grep ${GENE} GENOME_TREE/OrthoFinder_out/Results_Nov22/Orthogroups/Orthogroups.txt | python /global/scratch/users/annen/KVKLab/Gene_tree_topology/filter_og.py > treeKO_analysis/NUC_TREES/${OG}_genes.txt
