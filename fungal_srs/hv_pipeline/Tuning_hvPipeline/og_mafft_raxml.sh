...
cd /global/scratch/users/annen/000_FUNGAL_SRS_000/Tuning_hvPipeline/REFINEMENT_nlrs/Refinement2_init_corrected
echo "*********** ${OG} starting raxml ***********"
dt=$(date '+%m/%d/%Y %H:%M:%S')
echo "STARTING: ${dt}"
raxmlHPC-PTHREADS-SSE3 -s ${OG}.afa -n RAxML.${OG} -T ${SLURM_NTASKS} -m PROTCATJTT -f a -x 12345 -p 12345 -# 100
dt=$(date '+%m/%d/%Y %H:%M:%S')
echo "DONE: ${dt}"
