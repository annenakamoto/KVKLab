### Run auto-refinement exactly as in Daniil's script, except not filtering for hv alignment cutoffs (to get all possible split clades)
...
option_list = list(
  make_option(c("-d", "--working_dir"), type="character", default=NULL, 
              help="working directory path, where Cladesplit and Initial directories are located [REQUIRED]", metavar="character"),
  make_option(c("-i", "--init_dir"), type="character", default=NULL, 
              help="name of initial directory containing the trees and alignments to cladesplit [REQUIRED]", metavar="character"),
  make_option(c("-c", "--cls_dir"), type="character", default=NULL, 
              help="name of the directory to output the results of the cladesplit to [REQUIRED]", metavar="character"),
...
if (is.null(opt$cls_dir)){
  print_help(opt_parser)
  stop("Cladesplit directory is required", call.=FALSE)
}
...
if(!dir.exists(cls_dir)){dir.create(cls_dir)}
